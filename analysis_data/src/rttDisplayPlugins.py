...
class RttDisplay:
    ...
    def __init__(self, dataID):
        ...
        #hange courrent working directory to the folder of data
        path = []
        ...
        #Check picture path dose not exists then creat a new folder 
        self.picturesPath = self.targetPath + '/pictures'
        self.htmlPath = self.targetPath + '/figuresHtml'
        ...
        with open('jsonData/information.json', 'r') as fp:
            self.info = json.load(fp)
        self.nbProbes = self.info['nbProbes']
        self.nbTimes = self.info['nbTimes']

        """Attributes json files to read data """
        self.rtt3AllDataFile = JsonFile( 'jsonData/AllData.json')
        self.rtt3ShortestTimeFile = JsonFile( "jsonData/ShortestTime.json")
        self.rtt3LengthPathFile = JsonFile( "jsonData/ShortestPathLength.json")
        self.rtt3DifferenceFile = JsonFile('jsonData/Difference.json')
        self.rtt3DiffPercentFile = JsonFile('jsonData/DiffPercent.json')
        self.rtt3MaxdiffPercentFile = JsonFile('jsonData/MaxdiffPercent.json')
        self.rtt3MindiffPercentFile = JsonFile('jsonData/MindiffPercent.json')
        self.rtt3MeandiffPercentFile = JsonFile('jsonData/MeandiffPercent.json')
        self.rtt3MeanDelayFile = JsonFile('jsonData/MeanDelay.json')
        self.rtt3MeanShortestDelayFile = JsonFile('jsonData/MeanShortestDelay.json')
...
# show with mpld3 should change plt.show() by f.show(), too heavy 
...
def plotSrcFixedForVisu(rtt3Min, rtt3Mean, rtt3Max,  src, nbProbes, picturesPath, htmlPath):
    ...
    bars1 = ax.bar(x-width*1.5, minSrc, width, color='g', label='min percentage of improvement')
    bars2 = ax.bar(x-width*0.5, meanSrc, width, color = 'r', label='mean percentage of improvement')
    bars3 = ax.bar(x+width*0.5, maxSrc, width, color = 'b', label='max percentage of improvement')
    #scatter1 = ax.scatter(x,minSrc, s=50, marker = "o")
    #scatter2 = ax.scatter(x,meanSrc, s=50, marker = "o")
    #scatter3 = ax.scatter(x,maxSrc, s=50, marker = "o")
    ax.set_ylabel('percentage of improvement')
    ax.set_xlabel("id of destination")
    ax.set_xlim([-1, nbProbes])
    ax.set_ylim([-1, 100])
    ax.set_xticks(range(nbProbes))
    ax.set_title("Min Max Mean percentages of improvement from src :" + str(src)+ " to all destinations " )
    ...
    # add plugins for probilities
    labelsBinCenters1 = [round(bar.get_height(),2) for bar in bars1]
    ids1 = [utils.get_id(bar) for bar in bars1]
    plugins.connect(f, BarLabelToolTip(ids1, labelsBinCenters1))
    
    # add plugins for probilities
    labelsBinCenters2 = [round(bar.get_height(),2) for bar in bars2]
    ids2 = [utils.get_id(bar) for bar in bars2]
    plugins.connect(f, BarLabelToolTip(ids2, labelsBinCenters2))
    
    # add plugins for probilities
    labelsBinCenters3 = [round(bar.get_height(),2) for bar in bars3]
    ids3 = [utils.get_id(bar) for bar in bars3]
    plugins.connect(f, BarLabelToolTip(ids3, labelsBinCenters3))
...
def showDefaultTime(rtt3Data, src, dst, picturesPath,  htmlPath):
    ...
    if 2000 in rtt3Data:
        ...
        if src != dst:
            ...
            points = ax.plot(x, rtt3Data[src][dst].compressed(),'b',  marker='o', markersize = 3.0, label='shortest path time') 
...
def showHistoDefaultTime(rtt3Data, src, dst, picturesPath,  htmlPath):
    ...
    if src != dst:
        ...
        bins = np.linspace(int(min(data))-4, int(max(data))+4, 40)
        probilities, bins, patches = ax.hist(data, weights=weights,bins=bins,facecolor='b', label = 'Histogram')
        ax.set_xlabel("Direct delay time")
        ax.set_ylabel("Probability")
        #ax.set_ylim([0,1])
        ax.set_xlim([bins[0]-10, bins[-1]+8])
        ...
        '''
        mpld3.plugins.connect(f, tooltip)
        # add plugins for bincenters
        lab = np.around(probilities[1:], 3)
        tooltip1 = plugins.PointHTMLTooltip(scatter, labels=lab.tolist())
        mpld3.plugins.connect(f, tooltip1)
        # add plugins for patches----doesn't work
        tooltip2 = plugins.PointHTMLTooltip(patches, labels=lab.tolist())
        mpld3.plugins.connect(f, tooltip2)
        '''
        #mpld3.save_html(f, htmlPath+'/'+ name +'.html')
        mpld3.show()
